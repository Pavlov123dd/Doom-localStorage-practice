...
const renderFormRooms = () => {
    meetingFormRoom.innerHTML = getRooms()
        .map(item => `<option style="background-color:${item} " value="${item}">${item}</option>`)
        .join(``);
}
...
meetingForm.addEventListener(`submit`, e=>{
    ...
    if(meetingExist){
        promptBlock.innerHTML= `🥵 Meeting on ${newMeeting.day} at ${newMeeting.hour} on ${newMeeting.room} already exist!`
        setTimeout(removePromptBlock, 2000)
        return;
    }
    ...
    if(busyParticipants.length){
        busyParticipants.forEach(item => promptBlock.innerHTML=`👨🏻‍💻 ${item} already busy on ${newMeeting.day} at ${newMeeting.hour}!`);
        setTimeout(removePromptBlock, 2000)
        return;
    }
    ...
    promptBlock.innerHTML=`🥳 Meeting on ${newMeeting.day} at ${newMeeting.hour} on ${newMeeting.room} successfully created!`
    setTimeout(removePromptBlock, 2000)

});

const renderCaption = roomTitle => `<caption>Calendar for <span style="background-color: ${roomTitle}">${roomTitle}</span> room</caption>`;
